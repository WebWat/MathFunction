...
//    Assert.Equal("-x", (multiplyAll (convertToFunc "-x")).ToString())

//    // Default
//    Assert.Equal("x*x-x*1",        (multiplyAll (convertToFunc "x*(x-1)")).ToString())
//    Assert.Equal("x*x-x*1+-1*x+1*1",      (multiplyAll (convertToFunc "(x-1)*(x-1)")).ToString())
//    Assert.Equal("1*0-1*x+-1*x+-x*0+x*x+x*x",      (multiplyAll (convertToFunc "(1-x)*(-x-x)")).ToString())
//    Assert.Equal("1*x+1*2*x-1*1+-x*x+-x*2*x+x*1", (multiplyAll (convertToFunc "(1-x)*(x+2*x-1)")).ToString())
//    Assert.Equal("x*x*x+x*x*2+x*1*x+x*1*2+3*x*x+3*x*2+3*1*x+3*1*2",  (multiplyAll (convertToFunc "(x+1)*(x+2)*(x+3)")).ToString())
//    Assert.Equal("1*x*x+1*x*3+1*2*x+1*2*3+0*x*x+0*x*3+0*2*x+0*2*3+x*1*x+x*1*3+x*0*x+x*0*3+1*1*x+1*1*3+1*0*x+1*0*3",       (multiplyAll (convertToFunc "((1+0)*(x+2)+(x+1)*(1+0))*(x+3)")).ToString())
//    Assert.Equal("cos(x)*2*x^1*sin(x)+cos(x)*2*x^1*1+cos(x)*x^2*cos(x)+cos(x)*x^2*0", 
//        (multiplyAll (convertToFunc "(2*x^1*(sin(x)+1)+x^2*(cos(x)+0))*cos(x)")).ToString())
    
//    // Divide
//    Assert.Equal("1/x*x", (multiplyAll (convertToFunc "1/x*x")).ToString())
//    Assert.Equal("x*1/x", (multiplyAll (convertToFunc "x*1/x")).ToString())
//    Assert.Equal("x*1/x+x*x", (multiplyAll (convertToFunc "x*(1/x+x)")).ToString())
//    Assert.Equal("x*1/x+x*x", (multiplyAll (convertToFunc "(1/x+x)*x")).ToString())
//    Assert.Equal("x*1/x-x*x", (multiplyAll (convertToFunc "(1/x-x)*x")).ToString())
