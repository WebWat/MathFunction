...
//        || node.Operation = "*" || node.Operation = "^" || node.Operation = "/"
...
//    let rec recClear (last: string) (result: Node) =
//        let resultStr = result.ToString()
//        if last = resultStr then
//            result
//        else
//            recClear resultStr (findBranch result)

//    recClear (node.ToString()) (findBranch node)
...
//let rec toСommonDenominator (node: Node) =
//    let mutable numberators: string list = []
//    let mutable denominators: string list = []
//    let mutable x0 = 0.
...
//            match (node.Left.Value.Operation, node.Right.Value.Operation) with
//            | ("/", "*") ->                       
//                numberators <- List.append numberators [node.Left.Value.Left.Value.ToString()]
//                denominators <- List.append denominators [node.Left.Value.Right.Value.ToString()]
//                findBranch node.Right.Value
//            | ("/", "/") ->
//                numberators <- List.append numberators [node.Left.Value.Left.Value.ToString(); node.Right.Value.Left.Value.ToString()]
//                denominators <- List.append denominators [node.Left.Value.Right.Value.ToString(); node.Right.Value.Right.Value.ToString()]
//                ()
//            | ("/", _) ->
//                numberators <- List.append numberators [node.Left.Value.Left.Value.ToString(); node.Right.Value.ToString()]
//                denominators <- List.append denominators [node.Left.Value.Right.Value.ToString()]
//                ()
//            | ("*", "/") ->
//                numberators <- List.append numberators [node.Right.Value.Left.Value.ToString()]
//                denominators <- List.append denominators [node.Right.Value.Right.Value.ToString()]
//                findBranch node.Left.Value
//            | (_, "/") ->
//                numberators <- List.append numberators [node.Right.Value.Left.Value.ToString(); node.Left.Value.ToString()]
//                denominators <- List.append denominators [node.Right.Value.Right.Value.ToString()]
//                findBranch node.Left.Value
//            | _ ->
//                numberators <- List.append numberators [node.Left.Value.ToString(); node.Right.Value.ToString()]
...
//    let clearFuncs (node: Node) =
//        if numberators.Length = 0 then
//            node
//        else
//            let converted = List.map (fun x -> splitFunc( convertToFunc x )) numberators

//            node

//    //let rec reduce (left: string list) (right: string list) =
//    //    let mutable be = false
//    //    if left.Tail.Length = 0 then
            
//    //    if List.contains left.Head right then 
//    //        reduce 
//    //            left.Tail
//    //            Array.map (fun x -> 
//    //                if x = left.Head && not be then
//    //                    be <- true
//    //                    ""
//    //                else x) right
...
//    if denominators.Length = 0 then 
//        node |> openBrackets |> multiplyFuncs
//    else
//        let up = 
//            convertToFunc (String.Join ('*', List.map (fun x -> $"({x})") numberators))

//        // add for complex
//        let down =
//            convertToFunc (String.Join ('*', List.map (fun x -> $"({x})") denominators)) 

//        let a = clearFuncs up
//        let b = clearFuncs down
